...
function Scene(){
    ...
    const visibleFloors = useControls("フロア表示",
        {
            TokyoSt_1: { value: true, label: "1階"},
            TokyoSt_0: { value: true, label: "0" },
            TokyoSt_B1: { value: true, label: "地下1階" },
            TokyoSt_B2: { value: true, label: "地下2階" },
        })
    ...
    useEffect(() => {
        ...
        floorConfigs.forEach(async ({ floor, position, files, colors }) => {
            ...
            for (let i = 0; i < files.length; i++) {
                ...
                await loadGeoJSON(file, { fill: color, stroke: 0x0000ff }, (meshes) => {
                    //positionYは後から読み込んだGeoJSONが上に来るように積む
                    floorData.push({ name: file, positionY: i*0.05, meshes });
                });
            }
